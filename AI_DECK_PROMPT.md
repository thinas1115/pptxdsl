...
13. 異なる2種類の情報を同じページで対応させて読む必要がある場合だけ`split`を使う。左右の子typeは
    `bullets / cards / table / chart / image / diagram`から選び、各領域に役割を示す`heading`を書く。
    比率・座標は書かず、`split`を入れ子にしない。単純な2案比較は`twocol`、対応関係が不要なら2枚へ分ける。
...
- `table`
- `twocol`
- `split` (異なる2種類の情報を左右で対応させる。子typeは限定、入れ子禁止)
