...
2. 必須内容から、資料の章立てと各章の到達目標を先に整理する。次に各ページの主題、結論、根拠を
   章へ割り当てる。まだtypeやページ順をギャラリーへ合わせない。その後、
   `docs/type-selection-guide.md`に従い、意味に合う最小限の対応済みtypeへ
...
1. `CONTENT_SCHEMA.md` と同じJSON構造で出力する。資料要件にない任意フィールドは出力しない。
   特に `lead`, `note`, `actor`, `attribute`, `emphasis`, 表紙の日付・組織名・作成者・フッターは、必要性と値が明示された場合だけ書く。
2. `meta.title`, `slides[*].title`, `slides[*].kicker`, 本文はすべて「資料要件」の
   テーマ・想定読者・目的に合わせて新規作成する。
   すべての`slides[*].title`は、内容を探しやすい**名詞句または短い疑問形の見出し**にする。
   `研修の目的`、`前提知識`、`VLANとは`、`同一VLAN内の通信`のように主題だけを示し、
   `〜を理解する`、`〜できる`、`〜を改善する`のような結論・因果・行動を言い切る文章を使わない。
   結論やページの読み方が必要な場合は`lead`、詳細な根拠は本文へ書く。通常入力のvalidatorは、
   疑問形にも疑問符を付けない。読点・句点・改行や文末述語を含む文章型タイトルを拒否する。
   文章型タイトルを許可するのは、
   回帰検証ギャラリーを内部生成する場合だけである。
...
5. システム構成・クラウド構成・一般的なデータフローは `diagram` type を使い、`CONTENT_SCHEMA.md` の
...
7. 同じフレームやパケットが端末・装置内部・伝送区間を通る間の状態変化は`nw_protocol_flow`、
   ...
   `knowledge_check`を使う。状態変化を`nw_topology`の接続線へ押し込まず、これらを表・画像・箇条書きへ簡略化しない。
   ...
   付加されたものを`encapsulation`へ8文字以内の短い名称で必ず書き、`TAG`を一律に流用しない。
8. `bullets` は文字中心で複数項目を示す場合に使う。手順・優先順位・読み順がある場合は`style: "numbered"`、順序を持たない要点は`style: "bullet"`、実施項目と完了状態は`style: "checklist"`を選ぶ。各項目は`text`を持つobjectで書き、`checked`はチェックリストでのみ使う。
9. `cards` は、主結論と複数の独立した根拠、KPI・選択肢・事例など、各項目を比較できる場合に使う。読み順のある要点には使わない。サマリ・事例は `style: "editorial"`、KPIは `style: "metrics"` を選ぶ。各カードは`heading / body`のobjectで書き、KPI値は`value`へ分離する。情報量が少ないことだけを理由に使わず、フェーズ名や図のノードなど別の構造に属する要素をカード化しない。
10. 表紙以外では、タイトルと本文の間に要旨が必要な場合だけ任意の `lead` を使う。結論・前提・読み方を1〜2行で書き、タイトルの言い換えや本文の繰り返しは書かない。本文下部に結論・示唆・前提だけを置く帯やカードは作らない。不要なスライドにはフィールド自体を付けない。
11. 写真、イラスト、画面キャプチャなど、1枚のビジュアル自体が主メッセージになる場合は `image` を使う。
...
15. `process`は直線工程なら`steps`、分岐・合流・差戻しがあるなら`flow.nodes / flow.levels / flow.edges`を使う。
...
- 専門用語を定義せず、詳細図や設定手順から説明を始める。
- 提案資料風の結論文をページタイトルへ書き、主題を探しにくくする。
