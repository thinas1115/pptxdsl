...
from tests.fixtures.gallery.content_stress_patterns import (
    LARGE_AWS_DIAGRAM,
    LARGE_DIAGRAM,
    IMAGE_STRESS,
...
from slidegen.diagram_layout import ICON_SIZE, Layout
from slidegen.diagrams2 import PROGRAM_LINE_PT
from slidegen.generate_from_json import RENDER
from slidegen.image_slide import fit_image_layout
from slidegen.timeline_layout import fit_program_roadmap, pack_activities
...
def main():
    errors = validate(deepcopy(STRESS_PATTERN_DECK), allow_sample_content=True)
    assert not errors, "\n".join(errors)
