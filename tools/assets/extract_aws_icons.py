...
DEST = Path(__file__).resolve().parents[2] / "slidegen" / "assets" / "icons" / "aws"
DEST.mkdir(parents=True, exist_ok=True)

if len(sys.argv) != 2:
    raise SystemExit(
        "使い方: python -m tools.assets.extract_aws_icons <AWS公式アイコンデッキ.pptx>")
...
report_path = Path(__file__).resolve().parents[2] / "out" / "aws_extract_report.txt"
