...
LEADS = {
    ...
    "scope": "実施する範囲と対象外を分け、合意が必要な前提条件を確認します。",
    "summary": "背景から提案までの判断過程を短く整理し、結論へつなげます。",
    "paired_comparison": "左右を同じ評価軸で対応させ、方式ごとの優位点を判断します。",
    "mapping": "課題と施策の対応関係を示し、未対応や重複対応を確認します。",
    "swimlane": "担当レーンと工程フェーズを重ね、引き継ぎ箇所を確認します。",
    "sequence": "送受信者とメッセージ順を分け、作業時系列を確認します。",
    "concept": "用語の定義、判断に必要な要点、誤解しやすい境界を一続きで確認します。",
    "network": "物理機器と論理セグメントを重ね、リンクが運ぶ通信範囲を確認します。",
    "protocol_state_flow": "同じフレームまたはパケットの情報が、処理段階ごとにどう変わるかを追跡します。",
    "protocol_anatomy": "フレーム内の各フィールドを分解し、追加情報の位置と役割を確認します。",
    "code_lab": "設定例と確認観点を並べ、投入後に見るべき状態まで整理します。",
    "knowledge_check": "設問と選択肢を同じ構造で示し、理解した理由まで確認します。",
}
