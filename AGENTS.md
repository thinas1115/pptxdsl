...
2. 内容データを扱う場合は[AI_DECK_PROMPT.md](AI_DECK_PROMPT.md)、
   [CONTENT_SCHEMA.md](CONTENT_SCHEMA.md)、[docs/type-selection-guide.md](docs/type-selection-guide.md)を確認する。
...
- ブランチ名は作業内容を表す`feature/`、`fix/`、`docs/`を使う。AI製品名、モデル名、エージェント名を
  prefixや識別子として入れない。
...
- renderer、テーマ、ギャラリー内容を変更したときは、配布用の
  `examples/gallery/pattern_gallery.pptx`も再生成して差分へ含める。
...
接続図を変更した場合は、一覧確認に加えて原寸画像で接続点、線上ラベル、自己処理・差戻し経路、
その直後の線とラベルを局所確認する。線の一部がラベル背景に隠れていないことを経路単位で追う。

背景に接する矢羽根・工程帯・章帯は、塗り面または輪郭のどちらかで背景との境界を明示し、
`qa-surface-on-canvas:`品質マーカーを設定して`VIS-CONTRAST`検査へ通す。
