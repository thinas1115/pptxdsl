...
| `cards` | 2〜6件の同格な選択肢、事例、判断材料、KPIを比較する | 1つの用語の定義と要点、読み順のある要点、工程や図の要素を並べる | `concept`、`bullets`、`process`、`diagram` |
...
- 2〜6件の同格な項目に独立した見出しと説明があり、選択肢・事例・判断材料を比較する場合は`cards`。
- 1つの用語を定義し、その意味・特徴・境界を説明する場合は`concept`。主項目と補足項目を作る目的で`cards`を使わない。
