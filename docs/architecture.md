...
異なるrendererの内容を左右へ置く要件では、次の3案を比較した。

| 案 | 内容 | 評価 |
|---|---|---|
| A | 既存rendererへ領域引数を追加し、親から直接呼ぶ | 再利用率は高いが、全幅前提の既存配置へ広く影響する |
| B | 組み合わせごとに半幅専用rendererを作る | 既存への影響は小さいが、組み合わせ数だけ実装と検証が重複する |
| C | 親`split`が領域を決め、対応済み子レイアウタだけを領域内で動かす | 子の対応を段階的に増やせ、既存の全幅配置と責務を分離できる |

実装はCを採用する。`split_layout.py`が左右50:50の`ContentArea`を作り、子ごとに独立した
収容候補を評価する。子は割り当てられた`left / right / top / bottom`だけを参照し、反対側へ
侵入できない。`diagram`は領域対応済みの既存エンジンを使い、それ以外は半幅制約を明示した
小さな子レイアウタを持つ。通常rendererの全幅配置は変更しない。

入力で比率や座標は指定できず、`split`の入れ子も許可しない。対応外の子typeが必要な場合は、
既存rendererを無条件に呼び出さず、半幅での標準配置、余白圧縮、要素縮小、明示停止を実装してから
validatorの許可リストへ追加する。
