...
この文書は、`pptxdsl`がスライド内容とレイアウトをどのように分離し、提出可能な品質を
...
2. `validate_content.py`が許可フィールド、必須項目、件数、型、参照、未確定文言を検証する。
...
| schema validator | `content.json` | 許可フィールド、必須項目、件数、型、参照整合性の合否 | 見た目、内容の事実性 |
...
| L1 レイアウト計算 | ジャンル固有の位置・サイズ・配線計算 | `diagram_layout.py`、`org_layout.py`、`timeline_layout.py`、`image_slide.py`、`diagrams2.py`、`candidate_renderers.py`、各renderer | 同一ジャンル内 |
...
共通化の中心はL0、L2、L3である。L1はグリッド、ガント、散布、縦詰め、ツリーなど、
...
ロードマップでは期間軸と判定ポイントを計算し、フェーズ名と狙いを左見出し欄へ、
作業を期間バーへ配置する。狙い・作業・判定点は必要な場合だけ追加する。体制図では`levels`から
箱を配置し、隣接階層の報告関係を連結成分ごとの共有幹へまとめ、助言・横連携だけを別線で描く。
表はセル内容の実測から列幅を配分し、2軸図は点の周囲から衝突しないラベル位置を探索する。
工程図は直線工程と階層付き分岐工程を同じtype内の明示的な構造語彙で選び分ける。
これらを1つのアルゴリズムへ統合すると、設定項目と例外分岐が増え、
...
lead未指定時は標準の本文領域を返し、指定時だけ実測したleadの高さに応じて本文上端を下げる。
各rendererは固定の`BODY_TOP`ではなくこの領域から配置と収容候補を計算するため、lead追加で本文が
黙って押し出されることはない。lead自体が最小フォントでも収まらない場合も生成を停止する。

日本語では、句読点が行頭へ出ないように折り返し位置を調整する。PowerPoint側でも禁則が働くように、
日本語runへ`lang="ja-JP"`を設定する。計測と描画で異なるフォントや行間を使ってはならない。

文章の折り返しは、`textfit.py`の役割別ポリシーを使用する。

| 役割 | 対象 | 改行規則 | 最小文字と停止 |
|---|---|---|---|
| `natural` | スライドタイトル、見出し、カード見出し、ノード名 | 英単語・識別子・数値単位・連続カタカナを分断せず、短い最終行と助詞孤立を避ける | rendererの`min_pt`まで縮小し、自然な改行または幅・高さを満たせなければ`FitError` |
| `body` | lead、本文、箇条書き | 保護語を分断せず、最終行が本文幅の18%未満なら意味単位で再配置 | rendererの`min_pt`まで縮小し、幅・高さを満たせなければ`FitError` |
| `compact` | 表セル、線ラベル、短い属性値 | 保護語を分断せず、最終行の均等化は行わない。日本語だけ文字単位へフォールバック | rendererの`min_pt`まで縮小し、幅・高さを満たせなければ`FitError` |

`fit_text_or_raise()`は太字を既定で`natural`、通常本文を`body`として扱う。
狭い領域で`compact`を使う場合だけrendererが役割を明示する。役割を理由に`min_pt`を下げたり、
PowerPoint側の自動縮小へ委譲したりしない。
...
`image` typeでは、画像の縦横比を固定したまま本文枠へ収める。`contain`は画像全体を表示し、`cover`は
枠を満たすよう中央トリミングする。説明は共通headerのleadへ集約し、画像下のcaption/source枠は持たない。
leadで利用可能領域が減った場合も、裁量余白、画像の縮小、`FitError`の順で処理し、引き伸ばしや
フッターへの侵入を許可しない。任意の右下影は画像shapeの効果として付け、レイアウト寸法を変えない。
...
`bullets`、通常の`cards`、`two_column`は、`layout_fit.fit_vertical_stacks()`へ項目ごとの自然高さを
測る関数を渡す。共通部品は複数stackの最大必要高を計算し、標準gap、gap圧縮、文字縮小、
`FitError`停止を同じ順序で選ぶ。各rendererは文字幅、行間、固定見出し高、配置位置などの
ジャンル固有値だけを持ち、候補選択ループを重複実装しない。

### 図解の配置と配線

`diagram` typeは、列・行・ノード・コンテナ・エッジを入力として受け取り、`diagram_layout.py`が
座標を計算する。ノードは接続辺ごとのポートを持ち、エッジは明示されたチャネルを通る直角経路として
描画される。始点と終点の線分はノード辺へ垂直に接続し、viaが接線方向から始まる場合は短い垂直
スタブを自動挿入する。エンジンはコンテナ境界貫通、非直角・逆向き・短すぎる区間に加え、別エッジ同士の
重なり・T字接触・交差を描画前に検査する。同一ノードからの分岐と同一ノードへの合流だけは、
意図した共通幹線として許可する。描画はコネクタを先、ノードを後にし、上下のコネクタはアイコン外周から
ラベルの背後を直進させる。ラベル背景のマスクは文字の実測外形に最小限の余白だけを加え、周囲の線や
オブジェクトを広く隠さない。マスクで許容するのは線とラベルの重なりだけで、アイコン貫通や不正な接続は
描画前検証で停止する。
...
|---|---|---|---|
| schema検証 | `content.json` | 未知キー、未対応type、必須項目不足、件数超過、不正な参照、未確定文言 | 見た目、意味・事実の正しさ |
| エンジン自己検証 | レイアウト計算結果 | 配線方向、境界貫通、区間長、別エッジの接触、領域不足 | 色、文字の視認性 |
| PPTX機械検査 | 生成済みPPTX | テキスト・画像衝突、枠線貫通、表セル内の文字溢れ、全図形・表・グラフのはみ出し | グラフ内部のラベル衝突、線同士の交差、Z順、印象 |
