...
| `mapping` | 50 | 50 | 100 | 単純対応は直結し、交差・多対多は対応表へ切り替えて誤読を防止 |
| `swimlane` | 50 | 50 | 100 | レーン数・工程数・長い担当名へ追従し、引き継ぎ線と文字の衝突なし |
| `sequence` | 50 | 50 | 100 | 参加者数とメッセージ数へ追従し、同期・非同期・応答を判別可能 |
...
交差しない単純な対応は直線、多対多または交差する対応は中央の対応表へ自動で切り替える。
